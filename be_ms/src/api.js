...
const { mongoose, User, Room, Availability, Booking, normalizeDate, getDateRange } = require('./mongodb');
...
app.post('/api/v1/booking', auth, validate('booking'), validateDateRange, (req, res, next) => {
  ...
  try {
    ...
    const room = await Room.findOne({ _id: room_id, is_active: true }).session(session);
    if (!room) {
